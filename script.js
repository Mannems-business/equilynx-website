...
function initExplainer() {
  ...
  function updateLayoutVars() {
    scaleFactor = Math.min(1, Math.max(0.55, W / 640));
    if (W < 500) {
      centerY = H * 0.35;
    } else if (W < 768) {
      centerY = H * 0.40;
  ...
  function sceneThreat(p, t) {
    ...
    const bw = 96 * scaleFactor;
    const bh = 74 * scaleFactor;
    const shackleR = 26 * scaleFactor;
    const bx = cx - bw / 2;
    const by = cy - 6 * scaleFactor;
    glowDot(cx, cy + bh / 2 - 6 * scaleFactor, 90 * scaleFactor, `rgba(${ACC},0.12)`, `rgba(${ACC},0)`);
    ctx.lineWidth = Math.max(1.5, 3 * scaleFactor);
    ctx.strokeStyle = col;
    // shackle
    ctx.beginPath();
    ctx.arc(cx, by, shackleR, Math.PI, 0);
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(cx - shackleR, by);
    ctx.lineTo(cx - shackleR, by + 8 * scaleFactor);
    ctx.moveTo(cx + shackleR, by);
    ctx.lineTo(cx + shackleR, by + 8 * scaleFactor);
    ctx.stroke();
    // body
    ctx.beginPath();
    ctx.roundRect(bx, by + 6 * scaleFactor, bw, bh, 10 * scaleFactor);
    ctx.stroke();
    // keyhole
    ctx.fillStyle = col;
    ctx.beginPath();
    ctx.arc(cx, by + (6 + bh * 0.42) * scaleFactor, 6 * scaleFactor, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillRect(cx - 2.5 * scaleFactor, by + (6 + bh * 0.42) * scaleFactor, 5 * scaleFactor, 20 * scaleFactor);
    // crack grows
    if (p > 0.3) {
      const cp = (p - 0.3) / 0.7;
      ...
      ctx.beginPath();
      ctx.moveTo(cx, by + 12 * scaleFactor);
      ctx.lineTo(cx - 10 * cp * scaleFactor, by + 30 * scaleFactor);
      ctx.lineTo(cx + 8 * cp * scaleFactor, by + 48 * scaleFactor);
      ctx.lineTo(cx - 6 * cp * scaleFactor, by + 66 * scaleFactor);
      ctx.stroke();
    }
  }
  ...
  function drawNeuralMotif(cx, cy, scale, t) {
    ctx.save();
    ctx.translate(cx, cy);
    ctx.scale(scale * scaleFactor, scale * scaleFactor);
    const layers = [3, 4, 3];
  ...
  function sceneResearch(p, t) {
    const cy = centerY;
    const leftX = W < 500 ? W * 0.26 : W * 0.31;
    const rightX = W < 500 ? W * 0.74 : W * 0.68;
    drawAtomMotif(leftX, cy, 1, t);
    drawNeuralMotif(rightX, cy, 1, t);
    // link the two disciplines
    ctx.strokeStyle = `rgba(${ACC},${0.18 * Math.min(1, p * 2)})`;
    ctx.lineWidth = 1.2;
    ctx.setLineDash([4, 6]);
    ctx.beginPath();
    ctx.moveTo(leftX + 40 * scaleFactor, cy);
    ctx.lineTo(rightX - 50 * scaleFactor, cy);
    ctx.stroke();
    ctx.setLineDash([]);
  }
  ...
  function scenePQC(p, t) {
    ...
    const cols = 7;
    const rows = 5;
    const gw = 220 * scaleFactor;
    const gh = 150 * scaleFactor;
    ctx.save();
    ctx.translate(cx, cy);
    ...
    for (let i = 0; i <= cols; i += 1) {
      for (let j = 0; j <= rows; j += 1) {
        ...
        ctx.fillStyle = `rgba(${ACC},${a})`;
        ctx.beginPath();
        ctx.arc(x, y, 1.6 * scaleFactor, 0, Math.PI * 2);
        ctx.fill();
      }
    }
    ctx.restore();
    ...
    if (s > 0) {
      ...
      const sw = 96 * scaleFactor;
      const sh = 120 * scaleFactor;
      ctx.beginPath();
      ctx.moveTo(0, -sh / 2);
      ctx.lineTo(sw / 2, -sh / 2 + 26 * scaleFactor);
      ctx.lineTo(sw / 2, 12 * scaleFactor);
      ctx.quadraticCurveTo(sw / 2, sh / 2, 0, sh / 2 + 6 * scaleFactor);
      ctx.quadraticCurveTo(-sw / 2, sh / 2, -sw / 2, 12 * scaleFactor);
      ctx.lineTo(-sw / 2, -sh / 2 + 26 * scaleFactor);
      ctx.closePath();
      ctx.stroke();
      if (s > 0.6) {
        ...
        ctx.beginPath();
        ctx.moveTo(-20 * scaleFactor, 2 * scaleFactor);
        ctx.lineTo(-6 * scaleFactor, 20 * scaleFactor);
        ctx.lineTo(26 * scaleFactor, -20 * scaleFactor);
        ctx.stroke();
      }
      ctx.restore();
    }
  ...
  function sceneProduct(p, t) {
    ...
    const gearX = W < 500 ? W * 0.25 : W * 0.3;
    drawGear(gearX, cy, 30 * scaleFactor, 9, t * 0.0012, 0.35 + 0.2 * (1 - p));
    drawGear(gearX + 38 * scaleFactor, cy + 24 * scaleFactor, 20 * scaleFactor, 7, -t * 0.0018, 0.3 * (1 - p) + 0.1);

    // arrow research -> product
    ctx.strokeStyle = `rgba(${ACC},${0.4})`;
    ctx.lineWidth = Math.max(1.2, 2 * scaleFactor);
    const ax = W < 500 ? W * 0.43 : W * 0.46;
    const bx = W < 500 ? W * 0.57 : W * 0.56;
    ctx.beginPath();
    ctx.moveTo(ax, cy);
    ctx.lineTo(bx, cy);
    ctx.lineTo(bx - 6 * scaleFactor, cy - 5 * scaleFactor);
    ctx.moveTo(bx, cy);
    ctx.lineTo(bx - 6 * scaleFactor, cy + 5 * scaleFactor);
    ctx.stroke();

    // right: a deployable product cube locking in
    const px = W < 500 ? W * 0.75 : W * 0.7;
    const appear = Math.min(1, p * 1.4);
    ctx.save();
    ctx.translate(px, cy);
    ctx.globalAlpha = appear;
    const r = 36 * scaleFactor;
    // isometric-ish cube
    ...
    ctx.stroke();
    // "Rust" tag pulse
    glowDot(0, 0, (6 + 2 * Math.sin(t * 0.006)) * scaleFactor, "rgba(180,235,255,0.9)", `rgba(${ACC},0)`);
    ctx.restore();
  }
  ...
  function drawGear(cx, cy, radius, teeth, rot, alpha) {
    ...
    for (let i = 0; i < teeth; i += 1) {
      ...
      ctx.lineTo(Math.cos(a0) * (radius + 5 * scaleFactor), Math.sin(a0) * (radius + 5 * scaleFactor));
      ctx.lineTo(Math.cos(a1) * (radius + 5 * scaleFactor), Math.sin(a1) * (radius + 5 * scaleFactor));
  ...
  function sceneSovereign(p, t) {
    ...
    const barW = 80 * scaleFactor;
    const barH = 9 * scaleFactor;
    const barYStep = 14 * scaleFactor;
    ...
    for (let k = 0; k < 6; k += 1) {
      ...
      const rr = R - 14 * scaleFactor - (k % 2) * 16 * scaleFactor;
      glowDot(Math.cos(a) * rr, Math.sin(a) * rr, 4 * scaleFactor, "rgba(180,235,255,0.9)", `rgba(${ACC},0)`);
    }
    ctx.restore();
  }
  ...
  function sceneFinale(p, t) {
    ...
    for (let i = 0; i < n; i += 1) {
      ...
      const spread = lerp(160 * scaleFactor, 24 * scaleFactor, Math.min(1, p * 1.2));
      const x = cx + Math.cos(a) * spread * (0.6 + 0.4 * Math.sin(i));
      const y = cy + Math.sin(a) * spread * 0.5 * (0.6 + 0.4 * Math.cos(i));
      glowDot(x, y, 3 * scaleFactor, `rgba(${ACC},0.8)`, `rgba(${ACC},0)`);
    }
    glowDot(cx, cy, (30 + 6 * Math.sin(t * 0.005)) * scaleFactor, `rgba(180,235,255,${0.4 + 0.2 * p})`, `rgba(${ACC},0)`);
    // wordmark E
    const fontSz = Math.round(Math.max(24, 46 * scaleFactor));
    ctx.fillStyle = `rgba(230,248,255,${Math.min(1, p * 1.5)})`;
    ctx.font = `300 ${fontSz}px 'Raleway', sans-serif`;
