...
// =====================================================
// QUANTUM ATOM FIELD — subtle background behind the star-field
// (orbiting qubits / electron shells + faint entanglement links)
// =====================================================
function initQuantumBackground() {
  ...
  function makeAtoms() {
    atoms = [];
    const count = W < 720 ? 3 : 5;
    for (let i = 0; i < count; i += 1) {
      const orbits = [];
      const baseR = rand(46, 92);
      for (let o = 0; o < 3; o += 1) {
        orbits.push({
          rx: baseR * rand(0.95, 1.55),
          ry: baseR * rand(0.3, 0.58),
          tilt: rand(0, Math.PI),
          spin: rand(0.0015, 0.0055) * (Math.random() < 0.5 ? -1 : 1),
          angle: rand(0, Math.PI * 2),
          ea: rand(0, Math.PI * 2),
          espeed: rand(0.012, 0.032) * (Math.random() < 0.5 ? -1 : 1)
        });
      }
      atoms.push({
        x: rand(0.08, 0.92) * W,
    ...
    for (let i = 0; i < atoms.length; i += 1) {
      for (let j = i + 1; j < atoms.length; j += 1) {
        ...
        if (d < 380) {
          ...
          ctx.strokeStyle = `rgba(79,195,247,${0.055 * (1 - d / 380)})`;
          ctx.lineWidth = 0.6;
          ctx.stroke();
        }
      }
  ...
  function drawAtom(atom, t) {
    ctx.save();
    ctx.translate(atom.x, atom.y);
    ctx.scale(atom.scale, atom.scale);
    ctx.globalCompositeOperation = "lighter";

    const pulse = 0.5 + 0.5 * Math.sin(t * 0.002 + atom.pulse);

    const glow = ctx.createRadialGradient(0, 0, 0, 0, 0, 28);
    glow.addColorStop(0, `rgba(79,195,247,${0.2 + 0.12 * pulse})`);
    ...
    ctx.fillStyle = `rgba(190,235,255,${0.55 + 0.3 * pulse})`;
    ctx.beginPath();
    ctx.arc(0, 0, 2.6, 0, Math.PI * 2);
    ctx.fill();

    atom.orbits.forEach((orb) => {
      if (!reduce) {
        orb.angle += orb.spin;
        orb.ea += orb.espeed;
      }
      ctx.save();
      ctx.rotate(orb.tilt + orb.angle);
      ...
      const ex = Math.cos(orb.ea) * orb.rx;
      const ey = Math.sin(orb.ea) * orb.ry;
      const eg = ctx.createRadialGradient(ex, ey, 0, ex, ey, 7);
      eg.addColorStop(0, "rgba(170,230,255,0.85)");
